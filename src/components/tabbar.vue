<template>
        <footer>
            <ul>
                <router-link to="/index" custom v-slot="{ navigate, isActive }">
                    <li>
                    <span class="tabs-block__nav__item__link--icon home--icon" @click="navigate" :class="isActive ? 'homeActive' : ''"></span>
                    <span @click="navigate" :class="isActive ? 'homeSpanActive' : ''">首页</span>
                    </li>
                </router-link>
                <router-link to="/theatre/theatreList" custom v-slot="{ navigate, isActive }">
                    <li>
                    <span class="tabs-block__nav__item__link--icon theater--icon" @click="navigate" :class="isActive ? 'theaterActive' : ''"></span>
                    <span>影院</span>
                    </li>
                </router-link>
                <router-link to="/eticket" custom v-slot="{ navigate, isActive }">
                    <li>
                    <span class="tabs-block__nav__item__link--icon ticket--icon" @click="navigate" :class="isActive ? 'ticketActive' : ''"></span>
                    <span>票夹</span>
                    </li>
                </router-link>
                <router-link to="/center" custom v-slot="{ navigate, isActive }">
                    <li>
                    <span class="tabs-block__nav__item__link--icon my--icon" @click="navigate" :class="isActive ? 'myActive' : ''"></span>
                    <span>我的</span>
                    </li>
                </router-link>
            </ul>
        </footer>
</template>
<style lang="scss" scoped>
footer {
  box-shadow: 0px 0px 0.06667rem #ebebeb;
  position: fixed;
  z-index: 5;
  left: 0;
  bottom: 0;
  width: 100%;
  height: 1.30667rem;
  background: #fff;
  text-align: center;
  ul {
    display: flex;
    li {
      flex: 1;
      display: flex;
      flex-direction: column;
      i {
        margin-top: 5px;
        font-size: 25px;
      }
      span {
        font-size: 12px;
        color: #232323;
      }
    }
  }
}
.home--icon{
    background-image: url(data:image/png;base64,);
}
.homeActive{
    width: 1.12rem !important;
    height: 1.12rem !important;
    margin-top: 0.13333rem !important;
    background-image: url(/static/img/tab_icon_home_selected.3d60fa8.png);
}
.homeSpanActive{
  display: none;
}
.theater--icon{
    background-image: url(data:image/png;base64,);
  }
.theaterActive{
  background-image: url(data:image/png;base64,);
}
.ticket--icon{
    background-image: url(data:image/png;base64,);
}
.ticketActive{
  background-image: url(data:image/png;base64,);
}
.my--icon{
    background-image: url(data:image/png;base64,);
}
.myActive{
  background-image: url(data:image/png;base64,);
}
  .tabs-block__nav__item__link--icon{
    display: block;
    margin: 0 auto;
    width: 0.8rem;
    height: 0.8rem;
    background-position: center center;
    background-repeat: no-repeat;
    background-size: cover;
}
</style>
